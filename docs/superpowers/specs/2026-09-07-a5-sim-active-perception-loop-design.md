...
3. After stable arrival collect a NEW low-speed MID360 scan window (default 5 s). Flight arrival remains checked against the requested viewpoint. Sampling records one fresh in-bounds anchor and logs its difference from the request. During acquisition/recovery the anchor yaw remains fixed, but common low-speed position drift is allowed inside operating bounds; delayed stamped poses must remain within 0.10 m of the current position. Each PointCloud2 uses its OWN header-stamp T_map_sensor. Interrupted windows are discarded; unchanged sustained low-speed/yaw settling is required again within the original capture deadline. Express endpoints in the last accepted sensor frame/time for A2. Zero/no-return and nonfinite points are not evidence. No latest-TF substitution, high-speed/transit clouds or within-packet deskew. The window is an ordinary nonrepetitive-LiDAR acquisition setting. Its fixed-position timeout correction is documented in the [runtime design](2026-09-07-a5-low-speed-acquisition-design.md).
