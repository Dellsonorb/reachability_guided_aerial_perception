...
3. At each stable hover take a NEW PointCloud2, sensor XYZ + header-stamp T_map_sensor. Zero/no-return and nonfinite points are not evidence. Do not consume moving clouds, use latest TF as a timestamp substitute, or deskew airborne scans.
4. A2 accumulates per-frame endpoint votes; rebuild A3 from that snapshot; rank with frozen A4. Default max_viewpoints=3 counts initial observation and rescans. Stop on Score<=0, no predicted task gain, or the bounded observation count. Reaching the guard is not convergence or success.
...
- NPZ observation fields: `points_xyz`, `T_map_sensor`, scalar `stamp_s`, scalar `frame_id`.
