...
task list below retains the original test-first sequence. Starts 2–5 are complete
on `6cb5f0c`; all three tasks reaching Ground
handoff physically succeeded. The recorded start-2 bounds defect was minimally
fixed at `66c20aa`, and reserve start6 passed the complete natural task. No more
online starts are allowed in this batch. See `FINITE_SCAN_DEVELOPMENT_RESULTS.md`.
...
- [x] Finish start 1 and retain its actual result before production edits.
- [x] Add `tests/test_finite_scan.py`: use four one-ray packets hitting two cells;
...
- [x] Observe missing-model test failure, then implement
...
- [x] Run `PYTHONPATH=src:tests:scripts <core-python> -m unittest test_finite_scan`;
  check all geometry cases described in the design, then commit numerical code.

## 2. Shared ranking and runtime integration

- [x] Add failing tests for `rank_viewpoints(..., scan=...)` gain identity,
  unchanged inputs and Generic/Ours matching ranking on the same state.
- [x] Extend `NBVResult` with float opportunity and unoccluded opportunity arrays;
...
- [x] Update `src/reachability_guided_nbv/outputs.py` to save actual gain inputs and
...
- [x] Add optional path/window fields to `src/sim_active_perception/core.py`;
...
- [x] Test parser->worker duration/path, old default, nonexistent asset, both
  method decisions, and serialization. Run legacy tests, inspect diff and commit.

## 3. Replay and bounded online batch

- [x] Additional concrete consistency test: an AMBIGUOUS disk on the left edge
...
- [x] Test that finite scan does not first cull raw OCCUPIED cells when this
...
- [x] Change only current profile offsets to `[-2,-1,0,1,2]` after the documented
  offline common-candidate coverage finding; retain old/default profiles.

- [x] Independently review specification then code quality; resolve real issues.
- [x] Audit all eight existing Hard windows' actual scans/poses; compare old center
  and production finite forecasts on the four saved second-window decisions.
  Distinguish nominal pose from actual TF, phase horizon and known occlusion.
  Keep original outcomes; do not conflate the earlier50-packet prototype with
  the final51-packet publisher-filtered model.
- [x] Decide common lattice retention/refinement from offline coverage, not wins.
- [x] Freeze code/config before starts 2/3 (Hard01 Generic/Ours) and starts 4/5
...
- [x] Record `observation -> confirmed -> navigation -> refine -> D_exec ->
  grasp -> lift/hold`, windows/moves, sim time, missing-path data and first failure.
- [x] Compare predicted acquisition with actual support, preserve all failures,
  report remaining capability limits and fixed-version evaluation readiness.
- [x] Relevant regression, final diff/status, prepare development checkpoint for push;
