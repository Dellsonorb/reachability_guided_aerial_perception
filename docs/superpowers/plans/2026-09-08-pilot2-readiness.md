...
- [ ] RED tests for absent config and existing launcher omission:
...
- [ ] Create serialized config with root draw 202609082 and exact seeds
...
- [ ] Minimal launcher forwarding (v1 absence remains unchanged):
...
- [ ] Record config path/revision in new attempt metadata when explicitly supplied;
  no runtime target or allowance injected from setup metadata. RM4D-only remains
  original top-one, not operationally gated.
- [ ] Run `PYTHONPATH=src:tests CORE_PY -m unittest test_a6_pilot2 test_a6_attempt
...
- [ ] RED synthetic report fixture: one alias-only retained candidate, one true
...
- [ ] Implement `describe_round(decision, operational_summary, arrays)` as a pure
...
- [ ] Test missing reference, mixed class votes, no fabricated ground/confirmation,
...
- [ ] Obtain spec then quality review; exercise saved scoring diagnostics to
  confirm no effect on candidate generation, V, cost or Ground selection.

## Task 3 — preparation and independent setup (root)

- [ ] Read existing topic contracts; add only necessary passive control/RGB-D
...
- [ ] Run method-independent nominal depth check for all three serialized seeds.
...
- [ ] Inspect setup summaries and public geometry; preserve any activation.
