...
- [ ] Write tests for rectangle dimensions/rotation, overlap including contact,
  non-center overlap, yaw pi/4 corner separation, clipping and finite inputs.
- [ ] Run focused test module and confirm missing implementation failure.
- [ ] Implement FootprintSpec, footprint_vertices, footprint_cells. For each
...
- [ ] Run tests to green, then commit geometry and tests.
...
- [ ] Write tests for NaN/no support, blocked-only zero, unknown not blocking,
...
- [ ] Run focused module and confirm missing implementation failure.
- [ ] Implement typed output enums/records and pure builder using Task 1 SAT.
  Preserve all approved semantic qualifiers and provide covered-cell relations.
- [ ] Run tests to green, then commit core and tests.
...
- [ ] First test NPZ/JSON round trip, no support/NaN rendering, no backend switch,
...
- [ ] Implement field.npz, summary.json, supports.json, fixed-scale raw-cell
...
- [ ] Run focused tests to green. Generate results into `outputs/a3/`, inspect
  main PNGs and numeric support/score outputs. Commit this batch after review.

## Task 4: Handoff

- [ ] Write `docs/A3_TASK_RELEVANT_UNCERTAINTY.md` with equations, frame/footprint
...
- [ ] Bounded code review for algorithm/geometry/interface correctness only.
- [ ] Final test suite, compile new files, diff check and confirm no main-tracked
