...
- [ ] RED: reconstruct the existing two-candidate same-cell fixture from
...
- [ ] GREEN: implement a frozen WinnerAnchor record and stable loop:
...
- [ ] RED: call `build_task_uncertainty(field,belief,
...
- [ ] GREEN: use recovered source-ID anchor lookup only for the A3 xy/yaw
...
- [ ] RED/GREEN: make `candidate_catalog` use the same reconstruction. Add
...
- [ ] RED/GREEN: test all five policy routes and both numerical workers for
...
- [ ] Run `PYTHONPATH=src:tests CORE -m unittest test_exact_support
  test_a5_core test_task_uncertainty test_operational_gating
  test_operational_worker test_a6_policy -q` (use discovered actual A6 test name).
  Report observed RED failures and GREEN commands, inspect diff, commit core.
- [ ] Independent spec review, address findings; then independent code-quality
...
- [ ] RED/GREEN: build a pure diagnostic for winner-blocked/non-winner-unblocked
...
- [ ] Replay saved observations with existing `make_field`, `replay_observations`
...
- [ ] On slot15 Hard-002, assert source624 exact viable and unconfirmed with
...
- [ ] Evaluate A4 with identical current/candidates/visibility/cost on old/new
...
- [ ] Run focused replay tests and the standalone replay command; independently
  review outputs and all five original v1.1 gating regression groups.

## Task3 — natural SIM, review and checkpoint (root)

- [ ] Use exact original natural commands in `docs/OBJECT_AWARE_GATING_V11.md`,
...
- [ ] Inspect initial config, actual anchors, real observation windows, Ground
...
- [ ] Run full core suite and Noetic split, actual task-map tests as relevant,
  legacy slot15 reconstruction, A4 input/scoring checks and source-boundary diff.
- [ ] Write `docs/EXACT_POSE_SUPPORT_V12.md` with equations/API/tie rules,
...
- [ ] Independent final review; commit/push reviewed checkpoint, keep Draft PR.
  Stop for user review. No slot17, formal restart, new seeds or v1.2 statistics.
