...
def main():
    records = []
    profiles = {}
    for directory in sorted((HERE.parent / 'fixed-version').glob('slot-*')):
    ...
    pairs = []
    for scene in [s['id'] for s in read(ROOT / 'configs/fixed_version_paired.json')['scenes']]:
    ...
    result = dict(development_only=True, completed_starts=len(records), online_start_cap=14, planned_tasks=12,
        original_results_preserved=True, formal_statistics=False,
        same_version_pairs=pairs, records=records)
